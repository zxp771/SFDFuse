...
def upsample_and_convolve(input, scale_factor=2):
    # 使用双线性插值上采样图像
    upsampled_input = F.interpolate(input, scale_factor=scale_factor, mode='bilinear', align_corners=False)

    # 卷积核初始化为一个简单的平均滤波器，仅用作示例：实际使用时应根据任务选择合适的初始化
    convolution = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False)
    nn.init.constant_(convolution.weight, 1 / 9)

    # 应用卷积
    return convolution(upsampled_input)
######################################################
...
class Attention_MOAT(nn.Module):
...
class MOATBlock(nn.Module):
    def __init__(self, inplanes, planes, num_heads=8, stride=1, qkv_bias=False,
        ...
        self.attn = Attention_MOAT(planes, num_heads, qkv_bias, attn_drop, proj_drop)
...
# 加PoolMlp-end
class BaseFeatureExtraction_Pool(nn.Module):
...
class MlpHead(nn.Module):
    ...
    """
    def __init__(self, dim, mlp_ratio=4, act_layer=nn.GELU,#nn.GELU,SquaredReLU
        head_dropout=0.):
        super().__init__()
...
class GLFusion(nn.Module):
    ...
    def forward(self, x, y):
        ...
        pattn1 = sattn + cattn
        pattn2 = self.gelu(self.pa(initial, pattn1))  #gelu
        result = pattn2 * x + (1 - pattn2) * y  #initial +
        # result = self.conv(result)
        #result = initial * pattn2
        #result = self.conv(result)#####
        return self.gelu(result)#self.gelu(result + initial)#self.gelu(self.conv(result))#
...
###Dual Gated Feed-Forward
class FeedForward_Dual(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias):
        super(FeedForward_Dual, self).__init__()
...
## Dual-scale Gated Feed-Forward Network (DsGFF)
...
class MLP(nn.Module):
    def __init__(self, dim, mlp_ratio=4):
        ...
        self.norm = LayerNorm_ConvNeXt(dim, eps=1e-6, data_format="channels_first")
